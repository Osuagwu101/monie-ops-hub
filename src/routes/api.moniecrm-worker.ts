...
const authSafetyPrompt = [
  ...
  "NEVER click Forgot Username, Forgot password, Recover username, account recovery, Resend code, Try another way, Use a different verification method, or any other recovery/reset link.",
  "If MonieCRM shows a one-time code, verification code, OTP, authenticator app, or two-factor/MFA prompt asking you to enter a code, remain on that exact screen and take no further action: do not enter, guess, request, resend, or output the code's digits under any circumstance, and do not click any recovery or alternate verification link. Do NOT attempt to finish, complete, or end the task while this screen is showing -- ending the task now would lose it. Instead, on every step while this screen is showing, set your current goal/reasoning to begin with the exact text MONIECRM_VERIFICATION_REQUIRED followed by one short plain-language sentence describing what is shown, with no digits included, and otherwise wait by re-observing the page without interacting with any element.",
  "Submit the username/password pair at most ONCE in this task. If Login Failed, invalid credentials, temporarily suspended, or any other authentication error appears (other than the verification-code prompt handled above), STOP immediately. Do not retry credentials.",
