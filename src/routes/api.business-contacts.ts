...
export const Route = createFileRoute("/api/business-contacts")({
  server: {
    handlers: {
      POST: async ({ request }) => handleImport(request),
    },
  },
});
...
async function handleImport(request: Request) {
  ...
  if (!cloudUrl || !serviceRoleKey) {
    return response({ ok: false, error: "contact_cache_not_configured" }, 503);
  }
  ...
  if (
    !/^\d{4}-\d{2}-\d{2}$/.test(body.sourceReportDate) ||
  ...
  for (const contact of body.contacts) {
    if (!validContact(contact)) {
      return response({ ok: false, error: "invalid_verified_contact" }, 422);
    }
  }
...
function validContact(value: VerifiedContact) {
  return Boolean(
    ...
    ["EXACT_NAME_AND_TERMINAL", "EXACT_NAME_SINGLE_POS", "MANUAL_VERIFIED"].includes(
      value.matchMethod,
    ),
  );
}
