...
export function StaffAccountsSection({ refreshSignal = false }: { refreshSignal?: boolean }) {
  ...
  return (
    <SectionStack>
      ...
      <RowHeader
        title="Staff Support Agents"
        detail={`${staff.length} account${staff.length === 1 ? "" : "s"}`}
      />
          ...
          <View style={styles.rowBetween}>
            <View style={styles.flex}>
              ...
              <Text style={styles.muted}>
                Staff Support Agent · Created {dateOnly(member.createdAt)}
              </Text>
            </View>
      ...
      {config ? (
        <>
          <View style={styles.metricGrid}>
            ...
            />
            <Metric
              label="MonieCRM session"
              value={humanize(config.authState)}
              good={config.authState === "authenticated"}
            />
          </View>
          ...
          {workspace?.challenge?.pending ? (
            <Card tone="warning">
              ...
              <Text style={styles.body}>
                {workspace.challenge.message ?? "Enter the current MonieCRM verification code."}
              </Text>
              <Field
              ...
              />
              <ActionButton
                title={submitting ? "Submitting…" : "Submit code"}
                disabled={submitting || verificationCode.length < 4}
                onPress={() =>
                  void execute(
                    () => submitAutomationCode(workspace.challenge!.challengeId!, verificationCode),
                    "Verification code submitted securely.",
                  )
                }
              />
            </Card>
          ...
          <Card>
            <View style={styles.rowBetween}>
              ...
              <Switch
                ...
                trackColor={{ false: "#D0D5DD", true: BLUE }}
              />
            </View>
            <Field
              label="Moniepoint login URL"
              value={config.moniepointLoginUrl ?? ""}
              onChangeText={(value) => setConfig({ ...config, moniepointLoginUrl: value || null })}
              placeholder="https://…"
              autoCapitalize="none"
            />
            <Field
            ...
            <Field
              ...
              onChangeText={(value) => setConfig({ ...config, proxyCountryCode: value || null })}
              placeholder="ng"
              autoCapitalize="none"
            />
            <View style={styles.twoColumns}>
              <CompactField
                label="Morning audit"
                value={config.morningAuditTime}
                onChangeText={(value) => setConfig({ ...config, morningAuditTime: value })}
              />
              <CompactField
                label="Morning refresh"
                value={config.morningRefreshTime}
                onChangeText={(value) => setConfig({ ...config, morningRefreshTime: value })}
              />
              <CompactField
                label="Evening refresh"
                value={config.eveningRefreshTime}
                onChangeText={(value) => setConfig({ ...config, eveningRefreshTime: value })}
              />
              <CompactField
                label="Retry minutes"
                value={String(config.retryBackoffMinutes)}
                onChangeText={(value) =>
                  setConfig({ ...config, retryBackoffMinutes: Number(value) || 10 })
                }
                keyboardType="number-pad"
              />
            </View>
            <ActionButton
              title={submitting ? "Saving…" : "Save configuration"}
              disabled={submitting}
              onPress={() =>
                void execute(
                  () =>
            ...
            />
            <SecondaryButton
              title={submitting ? "Working…" : "Run retrieval now"}
              disabled={submitting || Boolean(activeRun) || !secretsReady}
              onPress={() => void execute(queueAutomationRun, "Manual retrieval queued.")}
            />
          </Card>
          ...
          <Card>
            ...
            <Text style={styles.body}>
              Only enter values you want to change. Saved secrets are never displayed again.
            </Text>
            <Field
              label={`Browser Use API key · ${workspace?.secrets?.browserUseApiKeyConfigured ? "configured" : "missing"}`}
              value={browserUseKey}
              onChangeText={setBrowserUseKey}
              secureTextEntry
              placeholder="Leave blank to keep current"
            />
            <Field
              label={`Moniepoint username · ${workspace?.secrets?.moniepointUsernameConfigured ? "configured" : "missing"}`}
              value={username}
            ...
            />
            <Field
              label={`Moniepoint password · ${workspace?.secrets?.moniepointPasswordConfigured ? "configured" : "missing"}`}
              value={password}
            ...
            />
            <ActionButton
              title={submitting ? "Saving securely…" : "Save to Vault"}
              disabled={submitting || (!browserUseKey && !username && !password)}
              onPress={() =>
                void execute(async () => {
            ...
            <Card key={run.id}>
              <View style={styles.rowBetween}>
                <View style={styles.flex}>
                  ...
                  <Text style={styles.muted}>
                    {dateTime(run.createdAt)} · Attempt {run.attemptCount}
                  </Text>
                </View>
                <Pill
                  label={humanize(run.status).toUpperCase()}
                  good={run.status === "succeeded"}
                  bad={run.status === "failed"}
                />
              </View>
              {run.lastErrorMessage ? (
                <Text style={styles.errorText}>{run.lastErrorMessage}</Text>
              ) : null}
            </Card>
          ))}
        </>
...
export function ReadinessSection({ refreshSignal = false }: { refreshSignal?: boolean }) {
  ...
  const audit = async () => {
    ...
    try {
      ...
      setNotice("Readiness audit saved.");
      await refresh();
    } catch (caught) {
      setError(messageOf(caught));
    } finally {
  ...
  return (
    <SectionStack>
      <SectionIntro
        eyebrow="LIVE ACCEPTANCE"
        title="Readiness & Acceptance"
        body="See platform, manual-operations and live-automation readiness without hiding external activation gaps."
      />
      <Feedback error={error} notice={notice} />
          ...
          <Card tone={snapshot.overallStatus === "blocked" ? "error" : "success"}>
            <View style={styles.rowBetween}>
              <View style={styles.flex}>
                ...
                <Text style={styles.body}>
                  {score}% of checks pass · Generated {dateTime(snapshot.generatedAt)}
                </Text>
              </View>
              <Pill
                label={`${score}%`}
                good={snapshot.platformReady}
                bad={!snapshot.platformReady}
              />
            </View>
          </Card>
          <View style={styles.metricGrid}>
            ...
            <Metric
              ...
              value={snapshot.manualOperationsReady ? "Ready" : "Pending"}
              good={snapshot.manualOperationsReady}
            />
            <Metric
              label="Live automation"
              value={snapshot.liveAutomationReady ? "Ready" : "Pending"}
              good={snapshot.liveAutomationReady}
            />
            <Metric
              label="Scheduled retrieval"
              value={snapshot.automationEnabled ? "Enabled" : "Disabled"}
              good={!snapshot.automationEnabled || snapshot.liveAutomationReady}
            />
          </View>
          <ActionButton
            title={submitting ? "Running audit…" : "Run readiness audit"}
            disabled={submitting}
            onPress={() => void audit()}
          />
          <RowHeader
            title="Live system checks"
            detail={`${passed}/${snapshot.checks.length} pass`}
          />
          {snapshot.checks.map((check) => (
            <Card key={check.key}>
              <View style={styles.rowBetween}>
                ...
                <Pill
                  label={humanize(check.status).toUpperCase()}
                  good={check.status === "pass"}
                  bad={check.status === "blocker"}
                />
              </View>
          ...
          {history.map((auditItem) => (
            <Card key={auditItem.id}>
              <View style={styles.rowBetween}>
                ...
                <Pill
                  label={auditItem.platformReady ? "PLATFORM READY" : "BLOCKED"}
                  good={auditItem.platformReady}
                  bad={!auditItem.platformReady}
                />
              </View>
            </Card>
          ))}
...
export function OperationsTeamSection({ refreshSignal = false }: { refreshSignal?: boolean }) {
  ...
  const run = async () => {
    ...
    try {
      ...
      setResult(next);
      setNotice(
        next.mixCompliant ? "Amina's daily plan is ready." : "Amina produced a partial plan.",
      );
      await refresh();
    } catch (caught) {
      setError(messageOf(caught));
  ...
  const latestRuns = useMemo(() => {
    ...
    for (const item of workspace?.runs ?? [])
      if (!map.has(item.agentKind)) map.set(item.agentKind, item);
    return map;
  }, [workspace?.runs]);
  ...
  return (
    <SectionStack>
      <SectionIntro
        eyebrow="AUDITABLE ORCHESTRATION"
        title="Operations Team"
        body="Run and inspect Amina, Emeka, Zainab and Tunde over the same secured evidence model used by the web portal."
      />
      <Feedback error={error} notice={notice} />
      <Card>
        <CardTitle>Run the operations team</CardTitle>
        <Text style={styles.body}>
          Re-running replaces only untouched Amina-generated tasks. Started and manual work is
          preserved.
        </Text>
        ...
        >
          {(workspace?.assistants ?? []).map((assistant: AssistantOption) => (
            <Pressable
              key={assistant.id}
              style={[styles.selectPill, assistantId === assistant.id && styles.selectPillActive]}
              onPress={() => setAssistantId(assistant.id)}
            >
              <Text
                style={[
                  styles.selectPillText,
                  assistantId === assistant.id && styles.selectPillTextActive,
                ]}
              >
                {assistant.fullName}
              </Text>
        ...
        />
        <ActionButton
          title={submitting ? "Running team…" : "Run operations team"}
          disabled={submitting || !assistantId}
          onPress={() => void run()}
        />
      </Card>
      {result ? (
        <Card tone={result.mixCompliant ? "success" : "warning"}>
          <CardTitle>
            {result.totalCalls}/{result.dailyCallTarget} calls ready
          </CardTitle>
          <Text style={styles.body}>
            {result.taCalls} TA · {result.nonTaCalls} non-TA · {result.contactGaps} contact gaps
          </Text>
        </Card>
      ) : null}
        ...
        {["amina", "emeka", "zainab", "tunde"].map((agent) => {
          ...
          return (
            <View style={styles.agentCard} key={agent}>
              ...
              <Pill
                label={latest ? humanize(latest.status).toUpperCase() : "NO RUN"}
                good={latest?.status === "completed"}
                bad={latest?.status === "failed"}
              />
            </View>
          );
        })}
      ...
      <RowHeader
        ...
        detail={`${workspace?.recommendations.length ?? 0} open`}
      />
      {!loading && !workspace?.recommendations.length ? (
        <Empty text="No open recommendation for this plan date." />
      ) : null}
        ...
        <Card key={recommendation.id}>
          <View style={styles.rowBetween}>
            ...
            {recommendation.score !== null ? (
              <Pill label={`${Math.round(recommendation.score)} SCORE`} />
            ) : null}
          </View>
          {recommendation.merchantName ? (
            <Text style={styles.itemStrong}>
              {recommendation.merchantName} · {recommendation.phoneNumber ?? "Phone missing"}
            </Text>
          ) : null}
          <Text style={styles.body}>{recommendation.rationale}</Text>
          {recommendation.talkingPoints ? (
            <Text style={styles.muted}>Talking points: {recommendation.talkingPoints}</Text>
          ) : null}
        </Card>
      ))}
    </SectionStack>
...
function RowHeader({ title, detail }: { title: string; detail: string }) {
  return (
    <View style={styles.rowBetween}>
...
function Feedback({ error, notice }: { error: string | null; notice: string | null }) {
  return (
    <>
      {error ? (
        <View style={[styles.feedback, styles.errorCard]}>
          <Text style={styles.errorText}>{error}</Text>
        </View>
      ) : null}
      {notice ? (
        <View style={[styles.feedback, styles.successCard]}>
...
function Field(props: ComponentProps<typeof TextInput> & { label: string }) {
  ...
  return (
    <View>
      ...
      <TextInput {...input} placeholderTextColor="#98A2B3" style={styles.input} />
    </View>
  );
}
function CompactField(props: ComponentProps<typeof TextInput> & { label: string }) {
  const { label, ...input } = props;
  return (
...
function Metric({ label, value, good }: { label: string; value: string; good: boolean }) {
  return (
    <View style={styles.metric}>
      ...
      <Text style={[styles.metricValue, good && styles.goodText]}>{value}</Text>
    </View>
  );
}
function Pill({
  ...
  return (
    <View style={[styles.pill, good && styles.goodPill, bad && styles.badPill]}>
      <Text style={[styles.pillText, good && styles.goodText, bad && styles.badText]}>{label}</Text>
    </View>
  );
}
...
function agentRole(agent: string) {
  if (agent === "amina") return "Operations Lead";
  if (agent === "emeka") return "TA & Merchant Growth";
  if (agent === "zainab") return "SME Lending";
