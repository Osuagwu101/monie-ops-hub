...
export default function App() {
  ...
  const loadData = useCallback(
    async (currentSession: Session, isRefresh = false) => {
...
function DirectorHome({
  ...
  const nextMeeting = useMemo(
    () => meetings.find((item) => item.status === "scheduled" && new Date(item.starts_at).getTime() > now),
    [meetings, now],
  );
      ...
      >
        ...
        {activeMeeting ? (
          <View style={[styles.card, styles.activeCard]}>
            ...
            <Text style={styles.heroBody}>Acknowledge as soon as you have joined. This stops the repeating alarm cycle.</Text>
          ...
          <View style={styles.card}>
            ...
            <Text style={styles.countdown}>{countdownText(new Date(nextMeeting.starts_at).getTime() - now)}</Text>
            {nextMeeting.series?.meeting_url ? (
              ...
              <Text style={styles.mutedSmall}>A meeting URL can be added from the web Meeting Centre.</Text>
            )}
          </View>
        ) : (
          <View style={styles.card}>
            ...
            <Text style={styles.heroBody}>Pull down to refresh after the Director Meeting Centre is updated.</Text>
          </View>
        )}
        ...
        <View style={styles.card}>
          ...
          />
          {notificationReadiness ? <Text style={styles.readinessNote}>{notificationReadiness.note}</Text> : null}
        ...
        <View style={styles.cardList}>
          {meetings
            ...
            .slice(0, 8)
            .map((meeting) => (
              <MeetingRow key={meeting.id} meeting={meeting} onAcknowledge={onAcknowledge} now={now} />
            ))}
        ...
        <View style={styles.footerCard}>
          ...
          <Text style={styles.footerBody}>4 minutes after start: repeated alarms until you acknowledge joining.</Text>
        </View>
      </ScrollView>
    </SafeAreaView>
  );
  ...
  return (
    <View style={styles.meetingRow}>
      ...
      <View style={styles.meetingInfo}>
        ...
        <Text style={styles.meetingMeta}>{weekdayLong(meeting.starts_at)} · {timeOnly(meeting.starts_at)}</Text>
        <Text style={styles.meetingStatus}>{meeting.status === "joined" ? "Joined acknowledged" : started ? "Started" : "Scheduled"}</Text>
      </View>
  ...
  return (
    <SafeAreaView style={styles.safe}>
      ...
      <View style={styles.loginPage}>
        ...
        <PrimaryButton title={busy ? "Signing in…" : "Sign in"} disabled={busy} onPress={() => void signIn()} />
      </View>
    </SafeAreaView>
  );
}
  ...
  return (
    <SafeAreaView style={styles.safe}>
      ...
      <View style={styles.centeredPage}>
        ...
        <Text style={styles.centerBody}>
          Set EXPO_PUBLIC_SUPABASE_URL and EXPO_PUBLIC_SUPABASE_ANON_KEY to the same Supabase project used by the web portal. For remote push, also set EXPO_PUBLIC_EAS_PROJECT_ID.
        </Text>
      </View>
    </SafeAreaView>
  );
...
function BrandMark({ small = false }: { small?: boolean }) {
  return (
    <View style={[styles.brandMark, small && styles.brandMarkSmall]}>
      <Text style={[styles.brandMarkText, small && styles.brandMarkTextSmall]}>M</Text>
    </View>
  );
}
...
function PrimaryButton({
  ...
  return (
    <Pressable style={[styles.primaryButton, disabled && styles.buttonDisabled]} onPress={onPress} disabled={disabled}>
      {disabled ? <ActivityIndicator color="#FFFFFF" /> : <Text style={styles.primaryButtonText}>{title}</Text>}
    </Pressable>
  );
}
...
function ReadinessRow({ title, ready, detail }: { title: string; ready: boolean; detail: string }) {
  return (
    <View style={styles.readinessRow}>
      ...
      <Text style={[styles.readinessState, ready ? styles.readyText : styles.pendingText]}>{ready ? "READY" : "PENDING"}</Text>
    </View>
  );
}
...
function messageOf(error: unknown) {
  return error instanceof Error ? error.message : "Something went wrong while synchronising the app.";
}
...
const styles = StyleSheet.create({
  ...
  brandMark: { width: 72, height: 72, borderRadius: 20, backgroundColor: BLUE, alignItems: "center", justifyContent: "center" },
  brandMarkSmall: { width: 44, height: 44, borderRadius: 13 },
  brandMarkText: { color: "#FFFFFF", fontSize: 42, fontWeight: "900", letterSpacing: -3 },
  brandMarkTextSmall: { fontSize: 25, letterSpacing: -2 },
  brandTitle: { color: INK, fontSize: 18, fontWeight: "800" },
  brandSubtitle: { color: MUTED, fontSize: 11, marginTop: 2 },
  signOutButton: { borderWidth: 1, borderColor: BORDER, borderRadius: 10, paddingHorizontal: 12, paddingVertical: 8 },
  ...
  card: { backgroundColor: "#FFFFFF", borderWidth: 1, borderColor: BORDER, borderRadius: 20, padding: 18, gap: 12 },
  activeCard: { borderColor: BLUE, borderWidth: 2, backgroundColor: "#F4F8FF" },
  cardList: { backgroundColor: "#FFFFFF", borderWidth: 1, borderColor: BORDER, borderRadius: 20, overflow: "hidden" },
  cardTopRow: { flexDirection: "row", alignItems: "center", justifyContent: "space-between", gap: 12 },
  pill: { alignSelf: "flex-start", backgroundColor: "#EAF1FF", borderRadius: 999, paddingHorizontal: 10, paddingVertical: 5 },
  ...
  primaryButton: { minHeight: 50, borderRadius: 14, backgroundColor: BLUE, alignItems: "center", justifyContent: "center", paddingHorizontal: 18 },
  primaryButtonText: { color: "#FFFFFF", fontSize: 15, fontWeight: "800" },
  buttonDisabled: { opacity: 0.65 },
  ackButton: { minHeight: 52, borderRadius: 14, borderWidth: 2, borderColor: BLUE, backgroundColor: "#FFFFFF", alignItems: "center", justifyContent: "center" },
  ...
  readinessWarning: { color: "#B54708", fontSize: 11, lineHeight: 17, backgroundColor: "#FFFAEB", padding: 10, borderRadius: 10 },
  divider: { height: 1, backgroundColor: BORDER },
  meetingRow: { minHeight: 82, flexDirection: "row", alignItems: "center", gap: 12, padding: 14, borderBottomWidth: StyleSheet.hairlineWidth, borderBottomColor: BORDER },
  meetingDateBox: { width: 48, height: 52, borderRadius: 12, backgroundColor: SURFACE, alignItems: "center", justifyContent: "center" },
  ...
  input: { height: 52, borderWidth: 1, borderColor: BORDER, borderRadius: 14, paddingHorizontal: 15, color: INK, backgroundColor: "#FFFFFF", fontSize: 15 },
